...
# 定义一个函数来计算相似度并更新数据库
def update_or_insert_question(user_input, department):
    # 连接到SQLite数据库
    conn = sqlite3.connect('data/user.db', check_same_thread=False)
    cursor = conn.cursor()
    # 查询所有的问题
    cursor.execute("SELECT question_id, question, times FROM patient")
    ...
    # 遍历所有问题计算相似度
    for question_id, question, times in questions:
        similarity = fuzz.ratio(user_input, question)
        if similarity > max_similarity:
            max_similarity = similarity
            max_question_id = question_id
            max_question_times = times

    # 如果相似度大于90%，更新times字段
    if max_similarity > 90:
        new_times = max_question_times + 1
        cursor.execute("UPDATE patient SET times = ? WHERE question_id = ?", (new_times, max_question_id))
        conn.commit()
        print(f"Updated question id {max_question_id} with new times {new_times}")
    else:
        # 否则插入新问题
        cursor.execute("INSERT INTO patient (question, department, times) VALUES (?, ?, ?)",
                       (user_input, department, 1))
        conn.commit()
        print(f"Inserted new question '{user_input}' into patient table")
        # 关闭数据库连接
        conn.close()

    # 加载预训练的 ChatGLM-6B 模型和分词器
tokenizer = BertTokenizer.from_pretrained("/home/caiqing/swufe/jan/Langchain-Chatchat/model/bert",
                                          trust_remote_code=True)
model = BertForSequenceClassification.from_pretrained("/home/caiqing/swufe/jan/Langchain-Chatchat/model/bert",
                                                      trust_remote_code=True).half().cuda()

DEPARTMENTS = ["内科", "外科", "肛肠科", "妇产科", "儿科", "眼科", "耳鼻喉科", "皮肤科", "肛肠科",
               "口腔科", "神经科", "心理科", "骨科", "泌尿科", "肿瘤科", "康复科"]

def predict_department(question):
    # 使用tokenizer对问题进行编码
    inputs = tokenizer.encode_plus(question, return_tensors="pt", max_length=128, truncation=True).to("cuda")
...
def dialogue_page(api: ApiRequest, is_lite: bool = False, initial_message: str = None):
    ...
    if initial_message != "":
        ...
        prompt = st.text_area(chat_input_placeholder,value = initial_message,key = "prompt")
    else:
    ...
    if prompt:
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            ...
            if dialogue_mode == "LLM 对话":
                ...
                # 使用模型推断科室
                department = predict_department(prompt)

                print(department)

                # 在处理完对话后，更新数据库
                update_or_insert_question(prompt, department)
